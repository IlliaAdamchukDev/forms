...
import { inputClickAction, buttonClickAction, areaClickAction, checkboxClickAction, selectClickAction, addFieldAction, deleteFieldAction } from './reducers/field/field.actions'
...
export class FormBuilderComponent {
  ...
  constructor(private store$: Store<FieldsState>, private auth: AuthService) {
    this.type$.pipe(takeUntil(this.notifier)).subscribe((type)=>{this.fieldName = type});
    this.fields$.pipe(takeUntil(this.notifier)).subscribe((fields)=>{
      ...
      fields.filter((el) => (el.id > 0 && el.type!=="button")).forEach(
        el => {
          newGroup[el.id] = el.styles.required ? new FormControl("",Validators.required) : new FormControl();
        }  
      )
      if (this.group) {
  ...
  clicks = {
    'input': (id: {id:number})=>{this.store$.dispatch(new inputClickAction(id));},
    'textarea': (id : {id:number})=>{this.store$.dispatch(new areaClickAction(id))},
    'button': (id : {id:number})=>{this.store$.dispatch(new buttonClickAction(id))},
    'checkbox': (id : {id:number})=>{this.store$.dispatch(new checkboxClickAction(id))},
    'select': (id : {id:number})=>{this.store$.dispatch(new selectClickAction(id))},
  }
  ...
  drop(event: CdkDragDrop<{fieldName: string, key: number}[]>) {
    ...
    if (event.container.data===this.fields){
      this.store$.dispatch(new deleteFieldAction({id: this.form[event.previousIndex].key}));
      this.form.splice(event.previousIndex,1);
      return;
    }
    this.form.splice(event.currentIndex, 0, { ...event.previousContainer.data[event.previousIndex]});
