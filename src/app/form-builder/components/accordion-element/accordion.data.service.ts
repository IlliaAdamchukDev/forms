...
import { selectCheckedId, selectFields } from '../../reducers/field/field.selectors';
...
export class AccordionDataService {
  ...
  constructor (private store$: Store<FieldsState>) {
    this.fields$.pipe(takeUntil(this.notifier)).subscribe((newFields)=>(this.fields = newFields));
    this.id$.pipe(takeUntil(this.notifier)).subscribe((id)=>{
      this.id = id;

      let el = this.fields.find((field) => (field.id===id));
      this.fieldStyles$.next(el?.styles ?? {
        'height' : "",
        'width' : "",
        'placeholder' : "",
        'required' : false,
